...
float cmdA, cmdB, cmdBremote,forceARemote,forceBRemote, errA = 0, errB = 0;
...
void loop() {
  ...
  cmdB = (1 / 50.0) * errB + 90;  // a virtual spring about the 90 deg origin
  // OBSERVE: *gently,slowly* pushing on A-> high reaction force, B->low69
  ...
  if (bTeleoperatingOverSerial) {
    ...
    //Serial.print("B");
    //Serial.println(cmdB);  // sends cmbB to distant location
...
void handleSerial() {
  ...
  while (Serial.available()) {
    ...
    switch (s[0]) {
      ...
      case 'A':
        forceARemote = s.substring(1).toFloat();
        //Serial.println("#Extracted Float:" + String(cmdB));
        break;

      case 'B':
        forceBRemote = s.substring(1).toFloat();
        break;
